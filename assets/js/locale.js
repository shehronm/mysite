...
(() => {
  ...
  window.MiraI18n={lang,t(key,values={}){return (lang==='ru'?(words[key]??key):key).replace(/\{(\w+)\}/g,(_,id)=>String(values[id]??''));}};
  ...
  try{
    ...
    if(requested===lang||lang==='ru')localStorage.setItem('mira-language',lang);
    else if(lang==='en'&&/^\/(?:index\.html)?$/.test(location.pathname)&&localStorage.getItem('mira-language')==='ru'){
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    document.querySelectorAll('[data-language]').forEach(link=>{
      ...
      link.addEventListener('click',()=>{
        ...
        try{localStorage.setItem('mira-language',link.dataset.language);}catch{}
      });
    });
  });
})();
